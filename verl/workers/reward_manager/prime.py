...
async def single_compute_score(evaluation_func, completion, reference, response_length, task, task_extra_info, executor, timeout=600.):
        ...
        tasks = [
            asyncio.wait_for(
                loop.run_in_executor(
                    executor,
                    partial(evaluation_func, task, completion, reference, response_length, task_extra_info)  # Ensure synchronous
                ),
                timeout=timeout)
        ]
...
async def parallel_compute_score_async(evaluation_func,
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        tasks_async = [
            single_compute_score(evaluation_func, completion, reference, response_length, task, task_extra_info, executor, timeout=300.)
            for completion, reference, response_length, task, task_extra_info in zip(completions, references, response_lengths, tasks, extra_info)
        ]
...
class PrimeRewardManager:
    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        if return_dict:
            return {"reward_tensor": reward_tensor, "reward_metrics": metrics}
        else:
